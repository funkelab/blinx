...
class ProteinDb:
    def __init__(self, filename):
        ...
        logger.info("Read %d proteins in %.3fs", self.num_proteins, time.time() - start)
        ...
        if "amino_acid_counts" in self.proteins[0]:
            ...
            self.amino_acid_counts = np.array(
                [protein["amino_acid_counts"] for protein in self.proteins],
                dtype=np.int32,
            )

            logger.info("Extracted amino acid counts in %.3fs", time.time() - start)
            ...
            self.amino_acid_counts = np.zeros(
                (self.num_proteins, AminoAcid.NUM_AMINO_ACIDS.value), dtype=np.int32
            )
            for i, protein in enumerate(self.proteins):
                self.amino_acid_counts[i] = count_amino_acids(protein["sequence"])

            logger.info("Counted amino acids in %.3fs", time.time() - start)

    def get_amino_acid_counts(self, amino_acids=None, num_samples=None):
        ...
        if num_samples is not None:
            sample_indices = np.random.choice(
                len(self.proteins), num_samples, replace=False
            )
